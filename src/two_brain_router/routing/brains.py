...
class GpuLocalBrain:
    ...
    **Unverified on this machine.** The `llama-server` OpenCL build and the
    ...
    #: Kept separate from _MAX_NEW_TOKENS: a structured reply wants to be short
    #: and parseable, a chat reply wants room to finish its argument.
    ...
    def _post_completion(self, payload: dict) -> dict:
        ...
        try:
            # Not a hardcoded 300: with _MAX_NEW_TOKENS at 4096 and ~21 tok/s,
            # a long reply can exceed that and would be cut off mid-answer.
            with urllib.request.urlopen(request, timeout=self._TIMEOUT_S) as resp:
