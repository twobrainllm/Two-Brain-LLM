...
> **Update — the choice has been made for us.** The cloud service has **no
> VLM**: its catalogue is `llm | text_to_image | embedding`, and the two Stable
> Diffusion entries generate images rather than read them. Sending an image to
> the 70B does not even error — it returns 200 and answers "I don't see an
> image", silently dropping it. See
> [`../data/cloud_ai100/_real_endpoint_log.md`](../data/cloud_ai100/_real_endpoint_log.md).
>
> So **the local Qwen3-VL-4B is the only vision capability in the system**, and
> an image-bearing query has no escalation target. Option 1 is now the default
> state rather than a choice; option 3 has nowhere to send the image; **option 2
> is the only way an image query can benefit from the cloud at all.** The
> privacy-preferred design and the only feasible design turn out to coincide.
>
> One caveat that survives: under option 2 the *description* is what escapes,
> generated by the local VLM. If it mentions a face or reads text off a
> document, that lands in the text path and is only as safe as `PIIGuard` —
> which today does not mask names.
